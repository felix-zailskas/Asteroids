...
public class AsteroidsPanel extends JPanel implements GameUpdateListener {
    ...
    private void drawShipInformation(Graphics2D graphics2D) {
        ...
        graphics2D.drawString(
                this.game.getSpaceship().getName(),
                (int) this.game.getSpaceship().getLocation().x - this.game.getSpaceship().getName().length(),
                (int) this.game.getSpaceship().getLocation().y + 35
        );
